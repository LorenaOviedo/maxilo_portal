...
function buscarPacientes() {
    const url = `${API_URL}?modulo=pacientes&accion=search` +
...
function mapearDatosPaciente(p) {
    ...
    const telefono = (p.contactos || []).find(c => [1,2,3,4,5].includes(parseInt(c.id_tipo_contacto)))?.valor || '';
    const email    = (p.contactos || []).find(c => parseInt(c.id_tipo_contacto) === 6)?.valor || '';
...
document.addEventListener('DOMContentLoaded', function () {
    document.getElementById('btnGuardarPaciente')?.addEventListener('click', function () {
        ...
        ['tipo_sangre','antecedentes_medicos','medicamentos','cirugias_previas','notas_historial'].forEach(campo => {
