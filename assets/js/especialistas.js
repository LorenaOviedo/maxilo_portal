...
const especialistaController = {
    ...
    _aplicarModoLectura(soloLectura) {
        ...
        document.querySelectorAll('#modalEspecialista input, #modalEspecialista select, #modalEspecialista textarea')
            .forEach(el => {
        ...
        document.querySelectorAll('#bodyEspecialidades .btn-accion.eliminar')
    ...
    async _cargarDatos(id) {
        try {
            const r    = await fetch(`${API_URL}?modulo=especialistas&accion=get_especialista&id=${id}`);
            const data = await r.json();
            ...
            CatalogTable.showNotification('Error al cargar datos del especialista', 'error');
        }
    },
    ...
    async guardar() {
        ...
        // ── Validaciones frontend (mismo patrón que pacientes.js) ────────
        const soloLetras = /^[a-záéíóúüñA-ZÁÉÍÓÚÜÑ\s]+$/u;
        const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
 
        const reglas = [
            {
                id: 'espNombre', label: 'Nombre(s)', requerido: true,
                regex: soloLetras, msgRegex: 'El campo "Nombre" solo debe contener letras',
            },
            {
                id: 'espApPat', label: 'Apellido paterno', requerido: true,
                regex: soloLetras, msgRegex: 'El campo "Apellido paterno" solo debe contener letras',
            },
            {
                id: 'espApMat', label: 'Apellido materno', requerido: false,
                regex: soloLetras, msgRegex: 'El campo "Apellido materno" solo debe contener letras',
            },
            {
                id: 'espFechaNac', label: 'Fecha de nacimiento', requerido: false,
                validar: (val) => {
                    const f      = new Date(val);
                    const minima = new Date(); minima.setFullYear(hoy.getFullYear() - 120);
                    if (f >= hoy)   return 'La fecha de nacimiento no puede ser igual o futura a hoy';
                    if (f < minima) return 'La fecha de nacimiento no parece válida';
                    return null;
                },
            },
            {
                id: 'espFechaCont', label: 'Fecha de contratación', requerido: false,
                validar: (val) => {
                    const f      = new Date(val);
                    const minima = new Date(); minima.setFullYear(hoy.getFullYear() - 80);
                    if (f > hoy)    return 'La fecha de contratación no puede ser futura';
                    if (f < minima) return 'La fecha de contratación no parece válida';
                    return null;
                },
            },
            {
                id: 'espTelefono', label: 'Teléfono', requerido: false,
                regex: /^\d{10}$/, msgRegex: 'El Teléfono debe tener exactamente 10 dígitos',
            },
            {
                id: 'espEmail', label: 'Correo electrónico', requerido: false,
                regex: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                msgRegex: 'El formato del correo electrónico no es válido',
            },
            {
                id: 'espCP', label: 'Código postal', requerido: false,
                regex: /^\d{5}$/, msgRegex: 'El Código postal debe tener exactamente 5 dígitos',
            },
        ];
 
        for (const regla of reglas) {
            const valor = this._getVal(regla.id);
            if (regla.requerido && !valor) {
                CatalogTable.showNotification(`El campo "${regla.label}" es obligatorio`, 'error');
                document.getElementById(regla.id)?.focus();
                return;
            }
        ...
        // Personal
        ['espNombre', 'espApPat', 'espApMat', 'espFechaNac', 'espFechaCont',
         'espCalle', 'espNumExt', 'espNumInt', 'espIdCp'].forEach(id => {
    ...
    async cambiarEstatus(id, nuevoEstatus, nombre) {
        ...
        if (!confirm(`¿${accion.charAt(0).toUpperCase() + accion.slice(1)} al especialista ${nombre}?`)) return;
    ...
    _limpiarFormulario() {
        ...
        ['espId','espNombre','espApPat','espApMat','espFechaNac','espFechaCont',
         ...
         'espEstado','espMunicipio','espTelefono','espEmail'].forEach(id => this._setVal(id, ''));
        document.getElementById('modalEspecialistaNombre').textContent = '';
        this._renderEspecialidades();
