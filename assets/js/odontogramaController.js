...
const odontogramaController = {
  ...
  async cargar(numeroPaciente = null) {
    ...
    document
      .querySelectorAll(
        "#app-odontograma input, #app-odontograma select, #app-odontograma textarea",
      )
      .forEach((el) => {
        el.disabled = false;
      });
  ...
  async _inicializar() {
    ...
    try {
      ...
      if (data.success) this._catalogos = data;
        ...
        console.warn(
          "odontogramaController: catálogos no disponibles",
          data.message,
        );
    } catch (err) {
  ...
  _procedimientosParaAnomalia(idAnomalia) {
    if (!idAnomalia) return [];

    const porAnomalia = this._catalogos?.procedimientos_por_anomalia ?? {};
    const filtrados =
      porAnomalia[idAnomalia] ?? porAnomalia[String(idAnomalia)];
    if (Array.isArray(filtrados)) return filtrados;

    const globales = new Set(
      (this._catalogos?.procedimientos_globales ?? []).map(Number),
    );
    return (this._catalogos?.procedimientos ?? []).filter((p) =>
      globales.has(Number(p.id)),
    );
  },

  _renderOpcionesProcedimientos(procedimientos, idActual = null) {
    return procedimientos
      .map(
        (p) =>
          `<option value="${p.id}" ${p.id == idActual ? "selected" : ""}>${escHtml(p.nombre)}</option>`,
      )
      .join("");
  },

  _poblarSelectProcedimientosPanel() {
    const selProc = document.getElementById("odontProc");
    const selAnom = document.getElementById("odontAnomalia");
    if (!selProc) return;

    const idAnomalia = parseInt(selAnom?.value || "0");
    const procedimientos = this._procedimientosParaAnomalia(idAnomalia);

    if (!idAnomalia) {
      selProc.innerHTML =
        '<option value="">Selecciona una anomalia primero...</option>';
      return;
    }

    selProc.innerHTML =
      '<option value="">Seleccionar...</option>' +
      this._renderOpcionesProcedimientos(procedimientos);
  },
  ...
  _poblarSelectsPanel() {
    ...
    if (selAnom && this._catalogos?.anomalias) {
      selAnom.innerHTML =
        ...
        this._catalogos.anomalias
          ...
          .join("");
      selAnom.onchange = () => this._poblarSelectProcedimientosPanel();
    }

    const divCaras = document.getElementById("odontCarasGrid");
    if (divCaras && this._catalogos?.caras) {
      divCaras.innerHTML = this._catalogos.caras
        .map(
          (c) => `
                <div>
    ...
    this._poblarSelectProcedimientosPanel();
  ...
  _poblarSelectEditarEstatus(idRegistro, idEstatusActual) {
    ...
    sel.innerHTML = this._catalogos.estatus
      .map(
        (e) =>
  ...
  _poblarSelectEditarProcedimiento(
    idRegistro,
    idProcedimientoActual,
    idAnomalia,
  ) {
    const sel = document.getElementById(`odontEditarProc_${idRegistro}`);
    if (!sel) return;

    const procedimientos = this._procedimientosParaAnomalia(
      parseInt(idAnomalia || "0"),
    );
    sel.innerHTML = this._renderOpcionesProcedimientos(
      procedimientos,
      idProcedimientoActual,
    );
  },
  ...
  _montarVue(numeroPaciente) {
    ...
    this._appInstance = createApp({
      setup() {
        ...
        function estadoDiente(numero) {
          ...
          if (regs.every((r) => r.estatus_hallazgo === "Tratado"))
            return "tratado";
          if (regs.some((r) => r.estatus_hallazgo === "En proceso"))
            return "anomalia";
          return "atencion";
        }
        ...
        function toggleEditarProcedimiento(
          idOdontograma,
          idProcedimientoActual,
          idAnomalia,
        ) {
          ...
          if (set.has(idOdontograma)) {
            ...
            setTimeout(
              () =>
                self._poblarSelectEditarProcedimiento(
                  idOdontograma,
                  idProcedimientoActual,
                  idAnomalia,
                ),
        ...
        async function guardarEstatus(idOdontograma) {
          const sel = document.getElementById(
            `odontEditarEstatus_${idOdontograma}`,
          );
        ...
        async function guardarProcedimiento(idOdontograma) {
          const sel = document.getElementById(
            `odontEditarProc_${idOdontograma}`,
          );
          const nuevoId = parseInt(sel?.value || "0");
          if (!nuevoId) {
        ...
        async function guardarRegistro() {
          ...
          if (!pacienteId) {
            mostrarNotif(
              "Guarda primero el paciente para registrar hallazgos",
              "error",
            );
            return;
          }
          ...
          if (
            !self
              ._procedimientosParaAnomalia(idAnomalia)
              .some((p) => p.id == idProcedimiento)
          ) {
            mostrarNotif(
              "El procedimiento no corresponde a la anomalia",
              "error",
            );
            return;
          }
    ...
    if (appEl._templateOriginal === undefined) {
      //Antes de montar, guarda el HTML original para poder resetearlo al desmontar
  ...
  async _actualizarProcedimientoEnServidor(
    idOdontograma,
    idProcedimiento,
    numeroPaciente,
  ) {
