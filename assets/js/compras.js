...
const compraController = {
  ...
  inicializar() {
    ...
    this._poblarSelect(
      "ocProveedor",
      CATALOGOS_OC.proveedores,
      "id_proveedor",
      "razon_social",
    );
    this._poblarSelect(
      "ocTipoCompra",
      CATALOGOS_OC.tiposCompra,
      "id_tipo_compra",
      "tipo_compra",
    );
    ...
    this._poblarSelect(
      "ocEstatus",
      CATALOGOS_OC.estadosOrdenCompra,
      "id_estatus_orden_compra",
      "estatus_orden_compra",
    );
    this._poblarSelect(
      "ocSelectProducto",
      CATALOGOS_OC.productos,
      "id_producto",
      "nombre_producto",
      (p) =>
        `[${p.codigo_producto}] ${p.nombre_producto}${p.marca ? " — " + p.marca : ""}`,
    );
  },
  ...
  _aplicarModoLectura(soloLectura) {
    document
      .querySelectorAll(
        "#modalCompra input, #modalCompra select, #modalCompra textarea",
      )
      .forEach((el) => {
  ...
  async guardar() {
    ...
    if (!folio) {
      CatalogTable.showNotification(
        "El folio de la orden es obligatorio",
        "error",
      );
      this._focusTab("tabOCDatos", "ocFolio");
      return;
    }
    ...
    if (!proveedor) {
      ...
      this._focusTab("tabOCDatos", "ocProveedor");
      return;
    }
    if (!tipo) {
      CatalogTable.showNotification("Selecciona el tipo de compra", "error");
      this._focusTab("tabOCDatos", "ocTipoCompra");
      return;
    }
    if (!fecha) {
      CatalogTable.showNotification(
      ...
      this._focusTab("tabOCDatos", "ocFechaEmision");
      return;
    }
    if (!moneda) {
      CatalogTable.showNotification("Selecciona la moneda", "error");
      this._focusTab("tabOCDatos", "ocMoneda");
      return;
    }
    if (this._detalle.length === 0) {
      ...
      cambiarTab("modalCompra", "tabOCDetalle");
      return;
    }
    ...
    formData.append(
      "accion",
      this._modoEdicion ? "actualizar_compra" : "crear_compra",
    );
  ...
  imprimirCompra() {
    ...
    const proveedor =
      document.querySelector("#ocProveedor option:checked")?.textContent || "—";
    const tipo =
      document.querySelector("#ocTipoCompra option:checked")?.textContent ||
      "—";
    const moneda =
      document.querySelector("#ocMoneda option:checked")?.textContent || "—";
    const estatus =
      document.querySelector("#ocEstatus option:checked")?.textContent || "—";
    ...
    ventana.document.write(
      '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">' +
        ...
        "<strong>Fecha de impresi&oacute;n:</strong> " +
        new Date().toLocaleDateString("es-MX") +
        "<br>" +
        "Dr. Alfonso Ayala G&oacute;mez</div></div>" +
        '<div class="titulo">Orden de Compra &mdash; ' +
        escHtml(folio) +
        "</div>" +
        ...
        '<div class="emisor-row"><span class="emisor-label">RFC:</span><span>AAGA8401031Y5</span></div>' +
        ...
        '<div class="emisor-row"><span class="emisor-label">Tel&eacute;fono(s):</span><span>55 4491 6895</span></div>' +
        "</div>" +
        '<div class="emisor-bloque">' +
        ...
        "</div>" +
        '<div class="info-grid">' +
        '<div class="info-item"><span class="info-label">Proveedor</span><span class="info-value">' +
        escHtml(proveedor) +
        "</span></div>" +
        '<div class="info-item"><span class="info-label">Tipo de compra</span><span class="info-value">' +
        escHtml(tipo) +
        "</span></div>" +
        '<div class="info-item"><span class="info-label">Fecha de emisi&oacute;n</span><span class="info-value">' +
        formatFecha(fechaEm) +
        "</span></div>" +
        '<div class="info-item"><span class="info-label">Entrega estimada</span><span class="info-value">' +
        formatFecha(fechaEnt) +
        "</span></div>" +
        '<div class="info-item"><span class="info-label">Moneda</span><span class="info-value">' +
        escHtml(moneda) +
        "</span></div>" +
        '<div class="info-item"><span class="info-label">Estatus</span><span class="info-value">' +
        escHtml(estatus) +
        "</span></div>" +
        "</div>" +
        "<table><thead><tr>" +
        ...
        '<div class="fila"><span>Subtotal:</span><span>' +
        fmt(totales.subtotal) +
        "</span></div>" +
        '<div class="fila"><span>IVA (16%):</span><span>' +
        fmt(totales.iva) +
        "</span></div>" +
        "</div>" +
  ...
  async cancelar(id, folio) {
    if (
      !confirm(
        `¿Cancelar la orden de compra "${folio}"? Esta acción no se puede deshacer.`,
      )
    )
      return;
  ...
  confirmarProducto() {
    ...
    if (!sel?.value) {
      CatalogTable.showNotification("Selecciona un producto", "warning");
      return;
    }
    ...
    if (this._detalle.some((d) => d.id_producto == sel.value)) {
      CatalogTable.showNotification(
        "Este producto ya está en el detalle",
        "warning",
      );
      return;
    }
    ...
    const prod = (CATALOGOS_OC.productos ?? []).find(
      (p) => p.id_producto == sel.value,
    );

    this._detalle.push({
      id_producto: parseInt(sel.value),
      nombre:
        prod?.nombre_producto ?? sel.options[sel.selectedIndex].textContent,
  ...
  _renderDetalle() {
    ...
    document.getElementById("ocDetalleJson").value = JSON.stringify(
      this._detalle,
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Folio a mayúsculas
  document.getElementById("ocFolio")?.addEventListener("input", (e) => {
    e.target.value = e.target.value.toUpperCase();
  });

  // Al seleccionar producto, pre-llenar precio del catálogo
  document
    .getElementById("ocSelectProducto")
